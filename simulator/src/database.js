...
const init = async () => {
    ...
    dialogs.innerHTML +=
        `<div class='guide-dialog scramble hidden'>
            <h1>${solutionData.title}</h1>
            <h2>${solutionData.username}</h2>
            <h3 class='italic'>${solutionData.posted.toString().split("T")[0]}</h3>
    ...
    for (const step in steps) {
        let dialog =
            `<div class='guide-dialog step${step} hidden'>
            <h1>${steps[step].name}</h1>`;

        if (steps[step].text.length > 0)
            dialog += `<p>${steps[step].text}</p>`;

        dialog +=
            `<p>Algorithm: ${steps[step].algorithm}</p>
            </div>`;
