...
import { solutionData } from "./database";
...
const nextPage = (cube) => {
  ...
  if (currPage === dialogs.length - 1 || cube.inProgress()) return;
  // move to the next page
  currPage++;
  setTimeout(() => {
    dialogs[currPage].className = dialogs[currPage].className.substring(
      0,
      dialogs[currPage].className.length - 7
    );
  ...
  if (currPage > 1) {
    cube.queueMoves(solutionData.steps[currPage - 2].algorithm);
  }
};
...
const prevPage = (cube) => {
  ...
  if (currPage <= 1 || cube.inProgress()) return;
  ...
  if (currPage > 0)
    cube.queueMovesReversed(solutionData.steps[currPage - 1].algorithm);
};
...
const setupUINavigation = (cube) => {
  ...
  for (let i = 1; i < dialogs.length; i++) dialogs[i].style.opacity = 0;
};
